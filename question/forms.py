...
class SubmissionComparisonForm(forms.Form):
	def __init__(self, question_id, submission_id, *args, **kwargs):
		...
		self.fields['choose_submissions'] = forms.ChoiceField(choices=[(sub.id, sub.student.user) for sub in StudentSubmission.objects.filter(question__id=question_id) if not sub.id==submission_id])